...
# ╔═╡ 1e633575-6546-4840-8394-8f28991352ee
using PlutoUI

# ╔═╡ 8d8cb15d-996e-4fb3-ab5d-13dab1ba1f75
using OrdinaryDiffEq

# ╔═╡ 7f30f153-35d0-4a44-ab21-39c40e83069e
using StaticArrays

# ╔═╡ 433e7c88-bcef-400e-b61e-113ff16e54bf
using CairoMakie

# ╔═╡ b3173017-6841-4977-9823-dbb8dab012b2
struct TRNmodel
    ...
    # DC pulses (uA/cm^2)
    i_dc::Float64
    i_dc_start::Vector{Float64} 
    i_dc_stop::Vector{Float64}  
    ...
    function TRNmodel(;
        ...
        return new(
            g_cat, g_nat, g_kd, g_nap, g_kt, g_k2, g_ar, g_l,
            E_NA, E_K, E_CA, E_AR, E_L, C,
            i_dc, i_dc_start, i_dc_stop,
            i_exp, i_exp_start, i_exp_decay,
            t_rise, t_fall, t_inh_rise, t_inh_fall,
            E_AMPA, E_GABA, A, tA, AI, tAI
        )
    end
end


# ╔═╡ 6f43566b-c09c-4937-ba14-053b97e7b08c
begin
...
# ╔═╡ 9ccb01f6-2309-4607-9922-b1837946226d
function dsim(u,p::TRNmodel,t)
    v, m_nat, h_nat, m_nap, m_kd, m_kt, h_kt, m_k2, h_k2, m_cat, h_cat, m_ar, ext_exc, ext_inh = u

    I = 0.0

    # Inputs
    ## Applied current
    I += iapp_func(0.0,p.i_dc,t,(p.i_dc_start,p.i_dc_stop))
    I += iexp_func(p.i_exp,p.i_exp_decay,t,p.i_exp_start)

    ## External Synapses
    ### AMPAergic
    A, vpre = extsyn_func(t,p.tA,p.A)

    ### GABAergic
    AI, vpreI = extsyn_func(t,p.tAI,p.AI)
    ...
    # channel currents
    I += (p.g_nat*(m_nat^3.0)*h_nat + p.g_nap*m_nap) * (v-p.E_NA)
    I += (p.g_kd*(m_kd^4.0) + p.g_kt*(m_kt^4.0)*h_kt + p.g_k2*m_k2*h_k2) * (v-p.E_K)
    I += (p.g_cat*(m_cat^2.0)*h_cat) * (v-p.E_CA)
    I += (p.g_ar*m_ar) * (v-p.E_AR)
    I += (p.g_l) * (v-p.E_L)

    # Synapses
    ## Excitatory input
    dext_exc = p.t_rise*k_syn(vpre)*(1.0-ext_exc) - p.t_fall*ext_exc
    I += A*ext_exc * (v-p.E_AMPA)

    ## Inhibitory input
    dext_inh = p.t_inh_rise*k_syn(vpreI)*(1.0-ext_inh) - p.t_inh_fall*ext_inh
    I += AI*ext_inh * (v-p.E_GABA)

    # Final equations
    dv  = (-1.0/p.C)*I

    @SVector [dv,dm_nat,dh_nat,dm_nap,dm_kd,dm_kt,dh_kt,dm_k2,dh_k2,dm_cat,dh_cat,dm_ar,dext_exc,dext_inh]
end


# ╔═╡ 1e5f7c75-35fe-4dc2-af7f-c03feca2bc35
@bind t_end PlutoUI.Slider(100:100:5000, default=1000, show_value=true)

# ╔═╡ 702fc63e-ace0-4688-ab21-73cca73b4b45
@bind var_i_dc PlutoUI.Slider(-2:0.05:2, default=0.0, show_value=true)

# ╔═╡ d7a18776-db46-4cd3-bd26-e952253eabfe
@bind var_leak PlutoUI.Slider(0.1:0.001:0.2, default=0.1, show_value=true)

# ╔═╡ 18c7ba84-edad-4424-a9a6-fb48a9a3174b
p = TRNmodel(
	i_dc=var_i_dc,
	i_dc_start=[200],
	i_dc_stop=[t_end-200],
...
# ╔═╡ 4e960914-e074-48c1-852b-328a2da69c71
begin
	tspan = (0, t_end)
	...
	sol = solve(prob, VCAB3(), saveat=0.01, save_idxs=1)
...
manifest_format = "2.0"
project_hash = "ac4a558186a74fe3a0d2ba11de1d2788bb32123b"
